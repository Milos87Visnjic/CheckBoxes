...
public class PlayerAccountStepDefs {
    ...
    public void userClicksOnSettings() {
        playerAccountPage.SettingsButton.click();
    }

    @And("user clicks on Edit Communications")
    public void userClicksOnEditCommunications() {
        playerAccountPage.EditCommunicationsButton.click();
    }

    @Then("Check if checkboxes are selected and if not click them")
    public void checkIfCheckboxesAreSelectedAndIfNotClickThem() {
        playerAccountPage.clickCheckBoxFutureContestsContains();
        playerAccountPage.clickCheckBoxEmailActiveNotifications();
        playerAccountPage.clickCheckBoxTextNotifications();
    }

    @And("Verify checkboxes are clicked")
    public void verifyCheckboxesAreClicked() {

        Assert.assertTrue(playerAccountPage.CheckBoxFutureContests.getAttribute("value").contains("true"));
        Assert.assertTrue(playerAccountPage.CheckBoxMarketingNotificationEmail.getAttribute("value").contains("true"));
        Assert.assertTrue(playerAccountPage.CheckBoxMarketingNotificationText.getAttribute("value").contains("true"));

        Assert.assertTrue(playerAccountPage.CheckBoxFutureContests.isSelected());
        Assert.assertTrue(playerAccountPage.CheckBoxMarketingNotificationEmail.isSelected());
        Assert.assertTrue(playerAccountPage.CheckBoxMarketingNotificationText.isSelected());
    }
}
